...
```{r,echo=TRUE}
lmfit_2008 <- lm(formula = SalePrice ~ `Gr Liv Area` + `Year Built` + `Lot Area`, ames_raw_2008)
summary(lmfit_2008)
```
...
```{r,echo=TRUE}
FEfit_2008 <- lm(formula = log(SalePrice) ~ log(`Gr Liv Area`) + log(`Year Built`) + log(`Lot Area`), ames_raw_2008)
summary(FEfit_2008)
```

Your answer:
I think add log to all of the predictors is a kind of feature engineering. I think this will help with the prediction.
...
Your answer:
After feature engineering, the standard error become smaller and the p-value become smaller means that the model after feature engineering is more significant.
